...
Load the `ezplot` and `loans` libraries. The former allows us to make nice looking ggplot2 plots easily. The latter contains the dataset. 
```{r}
library(ezplot)
library(loans)
```

Examine the unsecured personal loans (upl) dataset.
```{r}
str(upl, vec.len=3)
```
...
All variables are coded as numeric. Some shouldn't be. For example, we know the response variable is in fact a binary indicator. We thus change it to factor.
```{r}
upl$bad = as.factor(upl$bad)
```

We also need to change the following predictors to factors. But first, we change them to characters because we need to check if they have missing values.
```{r}
